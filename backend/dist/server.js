        ...
        try {
            ...
            if (message.type === 'search_users') {
                ...
                const matchingUsers = Array.from(users.entries())
                    .filter(([id, userInfo]) => userInfo.tag && userInfo.tag.toLowerCase().includes(query.toLowerCase()))
                    .map(([id, userInfo]) => ({
                    id,
                    ...userInfo
                }));
...
app.get('/api/users/tag/:tag', (req, res) => {
    ...
    const matchingUsers = Array.from(users.entries())
        .filter(([id, userInfo]) => userInfo.tag && userInfo.tag.toLowerCase() === tag)
        .map(([id, userInfo]) => ({
        id,
        ...userInfo
    }));
...
app.get('/api/users/search/:query', (req, res) => {
    ...
    const matchingUsers = Array.from(users.entries())
        .filter(([id, userInfo]) => userInfo.tag && userInfo.tag.toLowerCase().includes(query))
        .map(([id, userInfo]) => ({
        id,
        ...userInfo
    }));
