# Messenger - Local Privacy-First Messenger

Полнофункциональный мессенджер с локальным хранилищем, шифрованием и WebRTC для звонков.

## 🚀 Функции

- ✅ Локальное сохранение всех данных (IndexedDB)
- ✅ Двухфакторная аутентификация (TOTP)
- ✅ Шифрование сообщений (AES-256)
- ✅ WebRTC для голосовых и видеозвонков
- ✅ PWA - установка как приложение
- ✅ Полная конфиденциальность - сервер только передает сообщения

## 📦 Установка

1. **Установите Node.js** с https://nodejs.org/
2. **Перейдите в папку frontend:**
   ```bash
   cd messenger-app/frontend
   ```
3. **Установите зависимости:**
   ```bash
   npm install
   ```

## 🚀 Запуск

### Frontend (приложение)
```bash
cd messenger-app/frontend
npm run dev
```

Откроется на http://localhost:5173

### Backend (сервер WebSocket - опционально)
```bash
cd messenger-app/backend
npm install
npm run dev
```

Сервер откроется на http://localhost:8080

## 🌐 Деплой на Vercel + Railway

### Важное замечание
Vercel не поддерживает WebSocket-соединения напрямую. Поэтому для полнофункционального мессенджера вам нужно:

1. Разместить фронтенд (React приложение) на Vercel
2. Разместить бэкенд (WebSocket сервер) на другом сервисе, поддерживающем WebSocket (например, Railway, Render, AWS, DigitalOcean)

### Подготовка к деплою

1. Убедитесь, что ваш WebSocket сервер запущен где-то публично (например, на Railway)
2. Получите URL вашего WebSocket сервера (например, wss://my-messenger-backend-production.up.railway.app)

### Настройка Vercel

1. Зайдите на сайт vercel.com и создайте новую команду
2. Импортируйте этот репозиторий GitHub
3. В настройках проекта укажите:
   - Framework Preset: `Create React App` или `Other`
   - Root Directory: `frontend`
   - Build Command: `npm run build`
   - Output Directory: `dist`
4. В настройках проекта добавьте переменную окружения:
   - Key: `VITE_WS_SERVER_URL`
   - Value: URL вашего WebSocket сервера (например, wss://my-messenger-backend-production.up.railway.app)

### Настройка Railway для WebSocket сервера

1. Зайдите на сайт railway.app и создайте аккаунт
2. Нажмите "New Project" → "Deploy from GitHub repo"
3. Выберите ваш репозиторий
4. Railway автоматически обнаружит, что это Node.js проект, и выполнит сборку
5. В настройках проекта убедитесь, что:
   - Build Command: `cd backend && npm install && npm run build`
   - Start Command: `npm start`
   - Working Directory: `backend`
6. Railway автоматически задаст переменную PORT
7. После деплоя получите Public Domain URL (например, `https://my-messenger-backend-production.up.railway.app`)
8. Используйте этот URL в настройках Vercel как значение переменной `VITE_WS_SERVER_URL`

### Автоматическая сборка

Vercel и Railway автоматически выполнит:
- `npm install` 
- `npm run build` (команда из package.json)
- Размещение и запуск приложения

## 📁 Структура проекта

```
messenger-app/
├── frontend/
│   ├── src/
│   │   ├── components/     # React компоненты
│   │   ├── context/        # Управление состоянием
│   │   ├── pages/          # Страницы приложения
│   │   ├── services/       # Сервисы (storage, encryption, WebRTC)
│   │   ├── styles/         # CSS файлы
│   │   ├── types/          # TypeScript типы
│   │   ├── App.tsx         # Главный компонент
│   │   └── main.tsx        # Точка входа
│   ├── public/
│   │   ├── manifest.json   # Web Manifest для PWA
│   │   └── service-worker.js
│   ├── package.json
│   ├── tsconfig.json
│   └── vite.config.ts
└── backend/
    ├── src/
    │   └── server.ts       # WebSocket сервер
    ├── package.json
    └── tsconfig.json
```

## 🔐 Безопасность

- **TOTP 2FA**: Google Authenticator совместимая
- **AES-256**: Шифрование сообщений
- **SHA-256**: Хеширование паролей
- **Локальное хранилище**: Все данные на устройстве

## 📱 Использование как приложение

1. Откройте приложение в браузере
2. Поддерживаемые браузеры: Chrome 51+, Firefox 55+, Safari 15+, Edge 79+
3. Нажмите кнопку установки (будет предложено браузером)
4. Приложение будет добавлено на главный экран

## 🏗️ Технологический стек

- **Frontend**: React 18, TypeScript, Vite
- **Стиль**: CSS3 (Mobile responsive)
- **Хранилище**: IndexedDB
- **Реал-тайм**: WebSocket + WebRTC
- **Криптография**: crypto-js
- **Backend**: Node.js, Express, ws

## 📝 Лицензия

MIT License
